...
from app.utils import format_data, get_bool_arg
...
def build_query(data_id:int,tenant_id:int,query=None):
...
def load_index_filter(data_id:int,tenant_id:int):
    # curr_settings = global_state.get_settings(data_id) or {}
    data_instance = ArcticDBInstance(dataset_id=data_id,tenant_id=tenant_id)
    curr_settings = data_instance.settings
...
def build_query_builder(data_id:int,tenant_id:int):
...
def inner_build_query_builder(settings, query_builder):
    from app.column_filters import ArcticDBColumnFilter
...
def load_filterable_data(data_id,tenant_id:int, req, query=None, columns=None):
    filtered = req.get('filtered',False)
    data_instance = ArcticDBInstance(dataset_id=data_id,tenant_id=tenant_id)
    query_builder = build_query_builder(data_id,tenant_id=tenant_id)
    if filtered:
        data = data_instance.get_data(query_builder=query_builder, columns=columns)
    else:
        data = data_instance.get_data(columns=columns)
