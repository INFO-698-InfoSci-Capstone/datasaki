...
from app.translations import text
from app.utils import classify_type, apply, find_dtype_formatter


class ColumnBuilder(object):
    def __init__(self, data, column_type, name, cfg):
        self.data = data
    ...
    def build_column(self):
        return self.builder.build_column(self.data)
...
class NumericColumnBuilder(object):
    ...
    def build_column(self, data):
        left, right, operation = (
            self.cfg.get(p) for p in ["left", "right", "operations"]
        )
        left  = {"col":left}
        right = {"col": right}
