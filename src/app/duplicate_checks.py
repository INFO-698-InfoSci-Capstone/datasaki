...
from app.utils import dict_merge, grid_columns, grid_formatter, triple_quote
from app.charts.utils import build_group_inputs_filter
...
class DuplicateCheck(object):
    def __init__(self, data_id,tenant_id, check_type, cfg):
    ...
    def test(self):
        data = self.data_instance.get_data()
    ...
    def execute(self):
        ...
        try:
            ...
            curr_settings = self.data_instance.settings
            self.data_instance.settings = dict_merge(curr_settings, dict(startup_code=code))
            return self.data_instance._dataset_id
